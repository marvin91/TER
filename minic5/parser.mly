...
%{
  ...
  let loc e = mk_loc e (Parsing.symbol_start_pos (),Parsing.symbol_end_pos())
  let loc_i i e = mk_loc e (Parsing.rhs_start_pos i, Parsing.rhs_end_pos i)
  let loc_dummy e = mk_loc e (Lexing.dummy_pos, Lexing.dummy_pos)
...
%token LPAREN RPAREN LBRACKET RBRACKET POINT COMMA ARROW%token LBRACE RBRACE
%token IF ELSE FOR WHILE
%token SEMI RETURN EOF
...
%right NOT INCR DECR ADDR ustar uminus uplus
%left RPAREN ARROW POINT LBRACE
...
| t=typ; v=var; LPAREN; l=separated_list(COMMA,decl_var);
  RPAREN; b=block
        { let ty, i = unvar t v in
                Dfun (ty, i, l, Some b) }
;
...
| e=expr; ARROW; id=ident               { (Eacces (loc (Eunop(Deref, e)), id))   }
| e1=expr; ASSIGN; e2=expr              { (Eassign (e1,e2))     }
| id=ident; LPAREN; 
le=separated_list(COMMA, expr); RPAREN	{ (Ecall (id, le))      }
...
| LPAREN; t=typ; RPAREN; e=expr         { (Ecast (t, e))        }
//| ch=CHAINE                           { (Estring ch)          }
;
...
| SEMI					{ Sskip                  }
| e=expr; SEMI                          { (Sexpr e)              }
| IF; LPAREN; e=expr; RPAREN; i=instr   { (Sif (e, i, (loc Sskip))) }                
| IF; LPAREN; e=expr; RPAREN; i1=instr;
  ELSE; i2=instr                        { (Sif (e, i1, i2))             }
| WHILE; LPAREN; e=expr; RPAREN; i=instr{ (Sfor ([loc (Econst(Cint (Signed, Int, 0)))], e,
						 [loc (Econst(Cint (Signed, Int, 0)))], i))  }
| FOR; LPAREN; l1=l_expr; SEMI; e=expr; SEMI;
	l2=l_expr; RPAREN; i=instr; SEMI	{ (Sfor (l1, e, l2, i)) }
| FOR; LPAREN; e=expr; RPAREN; i=instr; SEMI	{ (Sfor ([], e, [], i)) }
| FOR; LPAREN; e=expr; SEMI; l=l_expr;
	RPAREN; i=instr; SEMI			{ (Sfor ([], e, l, i))	}
| FOR; LPAREN; l=l_expr; SEMI; e=expr; RPAREN;
	i=instr; SEMI				{ (Sfor (l, e, [], i))	} 
| b=block                               	{ (Sbloc b)             }
